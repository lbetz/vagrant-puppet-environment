    ...
    module V2
      class Root
        def provision(facter, os)
          case os[:family]
          when "redhat"
            vm.provision :shell, :path => 'deployment/scripts/redhat.sh', :args => [ os[:name], os[:release] ]
          when "debian"
            vm.provision :shell, :path => 'deployment/scripts/debian.sh', :args => [ os[:name], os[:release] ]
          when "suse"
            vm.provision :shell, :path => 'deployment/scripts/suse.sh', :args => [ os[:name], os[:release] ]
          when "windows"
            vm.provision :shell, :path => 'deployment/scripts/windows.ps1'
          end
