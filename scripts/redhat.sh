...
rpm -aq |grep puppet5-release 1>/dev/null || yum install -y "http://yum.puppet.com/puppet5/puppet5-release-${PKG}-${RELEASE}.noarch.rpm" &>/dev/null || raise_error
#rpm -aq |grep puppet6-release 1>/dev/null || yum install -y "http://yum.puppet.com/puppet6/puppet6-release-${PKG}-${RELEASE}.noarch.rpm" &>/dev/null || raise_error
